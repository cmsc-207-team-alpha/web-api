...
if(!isset($_GET['id']))
{
	...
	while($sp=mysqli_fetch_array($get))
	{
		...
		*/
		array_push($response, 'id' => $sp['id'], 'firstname' => $sp['firstname'], 'lastname' => $sp['lastname'], 'mobile' => $sp['mobile'], 'email' => $sp['email'], 'password' => $sp['password'], 'address' => $sp['address'], 'panicmobile' => $sp['panicmobile']);
